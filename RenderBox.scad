...
module render_box(data) { 
    ...
    apply_master_bounds(m_bw(data), m_bl(data), m_bh(data), m_c_rad(data), m_chamf(data)) { 
        difference() { core_tray_chassis(data);
        if (get_val(NEEDS_GROOVE, data, false)) { up(m_bh(data) - m_safe_lid(data) - 1.0) cuboid([m_bw(data) - sw + 0.1, m_bl(data) + 0.1, m_safe_lid(data) + 0.4], anchor=BOTTOM);
        } }
    } 
}
...
module render_flip_box(data) {
    ...
    bh = m_bh(data); sw = m_safe_wall(data); sl = m_safe_lid(data); noz = m_noz(data);
    sf = m_safe_floor(data);
    hinge_d = 4.0; clearance = 0.2;
    clip_wall = noz * 4;
    ...
    union() {
        ...
        // [v4.17] End spines plunged entirely to the floor (sf) to eliminate mid-air overhangs
        translate([-(bw - sw*2)/2 + sw/2, bl/2 - 0.5, sf]) cuboid([sw*3, hinge_y_offset + 1, axle_z + cc_z - sf], anchor=BOTTOM+FRONT);
        translate([(bw - sw*2)/2 - sw/2, bl/2 - 0.5, sf]) cuboid([sw*3, hinge_y_offset + 1, axle_z + cc_z - sf], anchor=BOTTOM+FRONT);
        
        // [v4.17] Center pillars plunged entirely to the floor (sf)
        if (cols > 1 && !skip_pillars) {
            for(i=[1:cols-1]) {
                translate([-int_w/2 + i*(int_w/cols), bl/2 - 0.5, sf])
                    cuboid([div_t, hinge_y_offset + 1, axle_z + cc_z - sf], anchor=BOTTOM+FRONT);
            }
        }
...
module render_double_flip_box(data) {
    ...
    clip_outer_d = hinge_d + (clearance*2) + (clip_wall*2);
    cc_z = clip_outer_d / 2; hinge_y_offset = (clip_outer_d / 2) + 0.5;
    spine_w = hinge_y_offset * 2 + hinge_d;
    axle_z = bh - cc_z;
    clip_len = bw - sw*6;
    ...
    union() {
        ...
        // [v4.17] End spines plunged entirely to the floor (sf)

    ...
    if (get_val(HAS_BUILTIN_GRID, data, false)) {
        ...
        if (type == JAR) {
            ...
            if (len(tok_x) > 0) {
                ...
                up(sf) {
                    for(i=[1:cols-1]) translate([-int_w/2 + i*(int_w/cols), 0, int_h/2]) cuboid([div_t, int_l, int_h], center=true);
                    for(j=[1:rows-1]) translate([0, -int_l/2 + j*(int_l/rows), int_h/2]) cuboid([int_w, div_t, int_h], center=true);
                }
            }
        }
    }
